...
## 76. Setup read the repo and never asked it anything

`/fx:setup` scanned for test commands and structure, drafted `repo.md`, and
presented it for review. Everything it knew came from reading code, and the
review gate was the only place a human entered.

That is the wrong shape for half of what setup needs, and `fx-brainstorm`
already states the rule it was missing: **facts are the agent's job, decisions
and vocabulary are the user's.** A test command is a fact and reading it is
correct. What a word means in this domain is not, and no amount of reading
settles it.

Setup now interviews in two clustered rounds before drafting, the same shape
`fx-brainstorm` uses: the domain (purpose in one line, the three or four terms
someone must use correctly, anything deliberate that looks like a mistake), then
the workflow (what must be true before a change ships, the base branch, where
worktrees go, what not to touch). Its own reading from step 1 is offered as the
first option each time, so the common answer costs one keystroke and the agent
is asking to be corrected rather than interviewing from nothing.

**`CONTEXT.md` becomes creatable, and only this way.** The original refusal to
generate it was right about inference and wrong about the whole file: a glossary
exists so a later session can catch the code drifting from what the user meant,
and one generated by reading that same code cannot perform that check against
itself. Answers make it legitimate. So setup writes it from round 1 if terms
resolved, and writes nothing if they did not, and says which.

`CLAUDE.md` and `AGENTS.md` stay as they were: one line pointing at `repo.md`.
Writing those files is the host's `init` and the user's call, and inlining the
project description into them would put it in every session's context, which is
the cost `repo.md`'s on-demand loading exists to avoid.
