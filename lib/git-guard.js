...
// `init` and `clone` create a NEW repository; they never write to the history
// or working tree of the checkout you are standing in. Blocking them made it
// impossible to start a new project from a session whose cwd is a main
// checkout — found by dogfooding, not by the test suite.
const ALLOWED_ON_MAIN = new Set(['fetch', 'worktree', 'remote', 'config', 'stash', 'init', 'clone']);
...
function stripHeredocs(command) {
  ...
  // ticket or a runbook that SHOWS a git command in an example is refused as
  // if the user had run it. Found by dogfooding — it blocked fx's own tickets.
...
module.exports = { inspect, splitSegments, stripHeredocs, parseGit, locate };
