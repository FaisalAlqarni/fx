...
// skill descriptions. It stays under 4,000 characters even with plan state at
// its worst: a repo.md and three plans with ledgers and long slugs
// (describePlans names at most three).
  ...
  + 'reason it stays short.';
{
  ...
  try {
    for (const harness of HARNESSES) {
      ...
      assert.ok(text.length < 4000, `${harness}: worst-case render is ${text.length} chars, must be under 4,000`);
