...
// ---------------------------------------------------------------------------
// 3. Write detection — ALLOWLIST first, denylist second (fix round 1)
// ---------------------------------------------------------------------------
//
...
//
// Bash is everything else. Round 1 shipped a DENYLIST over shell text:
// refuse a command that looks like a write. A review found the hole that
// shape always has: `bash -c`, `sh -c`, `python3 -c`, `node -e` — any
// interpreter — writes a file with no redirection and no listed write
// binary anywhere in the OUTER command text, so the denylist never saw any
// of them. Enumerating interpreters (`curl`, `wget`, `eval`, every
// scripting language on the machine) to deny is unwinnable — there is
// always one more.
//
// Round 2 inverts it. A read-only review agent runs a small, KNOWABLE set
// of commands: search, read, list, inspect history. ALLOWED_BINARIES is
// that set, enumerated from what agents/fx-lens-*.md and
// agents/fx-devils-advocate.md actually say they do ("Read the diff", "Grep
// for callers", "git diff", "find every place work enters a queue") — never
// from imagination. A command whose first word is not on that list is
// refused for that reason alone, which is what closes `bash -c`, `sh -c`,
// every interpreter, `curl`, `wget`, and `eval` WITHOUT any of them ever
// being named as bad: `bash -c "<anything>"` is refused because `bash` is
// the first word of that segment, whatever `-c`'s argument turns out to do.
//
// The denylist survives as a SECOND gate, for a binary that IS allowed but
// can still write: `git` is allowed (`git log` is a read) but `git commit`
// is not; `find` is allowed (`find . -name x` is a read) but `find -exec`/
// `-delete` is not; `cat`/`echo`/`printf` are allowed but redirected they
// write.
//
...
// this reuses. The allowlist closes the interpreter class outright (nothing
// not enumerated can run at all), which is the class a denylist could never
// close. What remains open: a binary already ON the allowlist that gains a
// write mode nobody has found yet (the `find -exec` class). Extend the
// per-binary second gate as real gaps turn up.
...
// re-opens exactly the "wrapper" hole this round closes). None of the six
// agent bodies require any of the four.
...
const FIND_WRITE = /\bfind\b[^|;&]*\s(?:-exec|-execdir|-delete|-fprintf|-ok|-okdir)\b/;
...
function scopeMustBeRefused(text, depth) {
  ...
  for (const seg of splitSegments(safe)) {
    ...
    if (!ALLOWED_BINARIES.has(firstWord(seg))) return true;

    // Gate 2: denylist, for an allowed binary that can still write.
    if (REDIRECT.test(blanked)) return true;
    if (GIT_WRITE_SUB.test(blanked)) return true;
    if (FIND_WRITE.test(blanked)) return true;
