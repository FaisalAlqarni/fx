...
//
// Task 14 fix round 2: git's parse-options accepts any unambiguous prefix of
// a long option and a short option's value attached, so the pattern follows
// that rule, not a list of spellings: `--out`, `--outp`, `--outpu` and
// `--output`, each bare or with `=value`, and `-o` bare or with its value
// attached (`-o/tmp/x`). `--o`/`--ou` are left alone: they are ambiguous
// with `--oneline`/`--ours`, which git refuses. (Measured on git 2.43: its
// diff options reject every abbreviation outright; other versions may not.)
const GIT_OUTPUT_FLAG = /^(?:--out(?:p(?:ut?)?)?(?:=.*)?|-o.*)$/;
