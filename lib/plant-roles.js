...
//      was. Keyed on agent_id in a private per-user directory, never the
//      repository.
...
const GENERATED_HEADER = '# generated by scripts/gen-codex-agents';
 ...
 * A same-named file without the generator's header, or a symlink, is the
 * user's own: it is left alone and reported `kept`.
 *
 * @returns {{written: string[], skipped: string[], stale: string[], kept: string[]}}
 */
function plantRoles({ home, source = DEFAULT_SOURCE } = {}) {
  ...
  for (const name of deriveReadOnlyAgents(source)) {
    ...
    let st = null;
    try { st = fs.lstatSync(destPath); } catch { /* not planted yet */ }
    ...
    // A symlink, or a file that does not carry the generator's header, is
    // the user's: never written through, never replaced.
    const existing = st.isFile() ? fs.readFileSync(destPath, 'utf8') : null;
  ...
  return { written, skipped, stale, kept };
}
...
// `agent_id`.
//
// Keyed on `agent_id` alone. Codex's agent_id is the child thread id, which
// is globally unique (research/codex.md, section 3), so no session scope is
// needed. The store used to be scoped by `process.ppid`, on the theory that
// every hook shared the Codex CLI as parent; under `dash -lc`, or a profile
// with an EXIT trap, the parent is a shell that differs per call, and every
// subagent came out unrecorded (final review I2).
//
// Where: `$CODEX_HOME/fx-agents` when CODEX_HOME is set, else
// `os.tmpdir()/fx-codex-agents-<uid>`: per user either way, because the temp
// area is shared by every local user. The directory is 0700 and must be
// owned by this user and not writable by anyone else; if it is not (another
// user created it first, or loosened it), nothing is recorded and every
// lookup returns null, which refuses writes. One file per agent_id, so two
// subagents starting at once never race on a shared file.
// `ponytail: records are never pruned; one small file per subagent. Add a
// prune-on-write if that ever matters.`

const SAFE_ID = /^[A-Za-z0-9][A-Za-z0-9._-]*$/;

function identityDir() {
  return process.env.CODEX_HOME
    ? path.join(process.env.CODEX_HOME, 'fx-agents')
    : path.join(os.tmpdir(), `fx-codex-agents-${process.getuid()}`);
}

// The directory exists, is a real directory, is ours, and only we can write it.
function privateDir(dir) {
  const st = fs.lstatSync(dir);
  if (!st.isDirectory() || st.uid !== process.getuid() || (st.mode & 0o077) !== 0) {
    throw new Error(`fx identity store ${dir} is not a private directory of this user`);
  }
  return dir;
}

function recordAgentIdentity({ agentId, agentType } = {}) {
  if (typeof agentId !== 'string' || !SAFE_ID.test(agentId)) return;
  const dir = identityDir();
  try {
    fs.mkdirSync(dir, { recursive: true, mode: 0o700 });
    fs.writeFileSync(path.join(privateDir(dir), `${agentId}.json`), JSON.stringify({ agentType: agentType || null }), { mode: 0o600 });
  } catch { /* best effort: an unrecorded agent is refused on a write, which fails closed */ }
}

function lookupAgentIdentity(agentId) {
  if (typeof agentId !== 'string' || !SAFE_ID.test(agentId)) return null;
  try {
    const raw = fs.readFileSync(path.join(privateDir(identityDir()), `${agentId}.json`), 'utf8');
...
function isWritingBashCommand(command) {
  if (typeof command !== 'string') return true;   // not text this classifier can clear
