...
//   (GIT_FLAGS is gone again since round 5, with git itself.)
//
...
//
//   Task 14 round 5: git is no longer an allowed binary (see the list's
//   comment below). A lens reviews from the diff file it is handed.
//
...
//     (design amendment A5). `git` was here until task 14 round 5 and is
//     gone: every exec path left was git reading config the review does not
//     control (diff.external, textconv, core.fsmonitor, and a nested repo's
//     config reached through the shell tool's `workdir`, which the hook
//     never sees). None of the binaries below reads repository config.
...
// FIND_ALLOWED_FLAGS, one list per binary: a flag
...
// it is checked as a flag (round 4 measured `git diff -U --ext-diff`, when
// git was still allowed, running diff.external this way). A short cluster (`-rn`) is read left to right; a value-taking
