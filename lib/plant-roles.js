...
function isWritingBashCommand(command) {
  ...
  // Redirection is tested against the whole heredoc-stripped, quote-blanked
  // command, NOT per split segment. splitSegments treats a bare `&` as a
  // command separator (backgrounding), which would sever `2>&1` into `2>` +
  // `1` and make the `(?!&)` lookahead below blind to the very `&` it exists
  // to see — turning the ordinary fd-duplication idiom into a false
  // positive.
  if (REDIRECT.test(blankQuoted(stripHeredocs(command)))) return true;
  ...
  return segments.some((seg) => {
    ...
    return WRITE_INPLACE.test(blanked)
      || GIT_WRITE_SUB.test(blanked)
      || PKG_WRITE.test(blanked)
