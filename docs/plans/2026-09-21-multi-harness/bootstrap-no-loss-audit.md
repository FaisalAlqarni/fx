...
| 74 | 117 | "A bug report names a symptom." | moved | `skills/fx-debug/SKILL.md`, Phase 5 (task 24), and `skills/fx-implement/implementer-prompt.md`, TDD rules, above Verify GREEN (task 27) | fx-debug is routed for every bug, and Phase 5 is the edit. A planned fix or refactor dispatched by fx-implement never passes through fx-debug, so the rule also sits in the implementer prompt, which every implementer reads at dispatch, before it writes code |
...
| 87 | 136-137 | "The smallest change in the wrong place is not lazy, it is a second bug." | moved | `skills/fx-debug/SKILL.md`, Phase 5 (task 24), and `skills/fx-implement/implementer-prompt.md`, TDD rules, above Verify GREEN (task 27), worded "The smallest change in the wrong place is a second bug." | The fix edit: fx-debug for a diagnosed bug, the implementer prompt for a planned fix or refactor, read at dispatch |
