...
Task 27: re-review of round 1 approved. Five evasions each fail the strict gate. Cleanup was by exact path this time.
Task 27: complete (commits c57c00e..03d1830, 1 fix round, review approved). Deferred to the final review: the release-gate throw when the merge-base has no manifest; the fx-audit:240 long line.
User cost review (2026-09-22): subagents were too expensive. I dispatched Opus for nearly all implementers and reviewers, and Opus babysitters woke about 50 times each with 130K+ context.
Ruling (user-prompted, saved to memory): from now on, Opus only for judgment-heavy work (security-critical code and review, design and spike analysis, red team, the final review). Sonnet for other implementers, reviewers, lenses, research and audits. Haiku for lookups and live-run runners. Babysitters wake only when the queue ends. After the final review, hand off to a fresh session for part B.
Correction: Haiku is not for live-run runners. Those copy credentials and run bypassed-permission sessions, and rules were broken even by stronger models. Haiku is only for read-only lookups and small edits where tests are already written.
Task 28 added (the user asked to make fx adhere): Claude Code tier names in the harness reference, a standard-tier default, a stated reason for the most capable tier, a cheapest-tier safety rule, a babysitter rule, and templates updated. Dispatched on Sonnet; review on Sonnet.
Task 28: DONE, eb83ede. Tiers are mapped in the Claude Code harness reference; model-selection.md gains the default, the stated-reason rule, the cheapest-tier safety rule and the babysitter rule; 4 templates updated, including a stray re-review placeholder. Agents/*.md pin models by a separate mechanism, flagged. Review dispatched on Sonnet.
...
Security re-check of round 2: CLEAN at Critical and Important (.fx/.../reports/security-round2-recheck.md). 3 Minor:
- (1) The accepted pipe residual is broader: a quota GAP fires on stderr text even at exit 0. The cheap narrowing is to GAP only on a clean non-zero exit.
- (2) A CLI install path can bind back a credential-holding part of the home (e.g. ~/.local for a standalone codex). Not the case on this host.
- (3) The allowlist skips top-level dot-dirs and files, and does not hide binds of /development onto kept dirs. Not the case on this host.
Ruling: fix all 3 in one small round AFTER the frozen matrix finishes. Why: the queue reads live.sh/jail.sh from the worktree on every row, so editing them mid-run would mix versions. None of the 3 changes the jail's behavior on this host, and (1) changes only GAP-vs-FAIL labels for a quota error. Cost if wrong: the matrix would not be on the exact final harness bytes. That is acceptable because only test-harness files change and their gate tests pin the change; caught by the exit-gate check-all.
Blocked on: the frozen matrix queue (tracked background command).
Frozen matrix, first pass: opencode 40 rows pass, 0 fail, 0 gap (matrix 18, plugin route 3, probes 7, row 04 10/10, lanes running). Claude Code GAPed every live row at 20:36 with a real API 429, "You've hit your session limit, resets 9:10pm". The hooks fired and the preamble was injected before the refusal, so this is quota, not a product defect. Re-running the CC phases at 21:17, after the reset, as final2-.
Claude Code re-run after the reset (final2-): matrix 16 pass / 0 fail / 2 gap (rows 13 and 14, the known user-invocability gap on this harness), probes 3 pass / 1 fail, row 04 5/5, lanes 9/9. The one FAIL is probe 93, review end to end: "no lens finding names app/users_controller.rb". The same probe passes on opencode. Diagnosis agent dispatched (Sonnet): product defect, probe defect, or flake.
Probe 93 FAIL diagnosed: probe defect, not a product defect. fx-review loaded and dispatched the security, database and devils-advocate lenses, and the security lens reported app/users_controller.rb:3 SQL injection, twice in the log. Claude Code dispatches agents asynchronously, so the finding arrives as a task_notification and a later assistant message, while tests/conformance/lib/events.js claude() only reads the tool_result of the dispatching tool_use_id and captured the "Async agent launched" placeholder. opencode dispatches synchronously, which is why it passes.
Ruling: fix events.js in the follow-up round, with the 3 security minors, then rerun row 93 on claude-code. Why: the harness mis-measures a working product. Cost if wrong: a real fx-review defect hides behind a parser change. Caught by the rerun, which must show the lens finding text and PASS.
Frozen matrix complete.
- opencode: 51 rows, 0 fail, 0 gap. Matrix 18, plugin route 3, probes 7, row 04 10/10, lanes 9/9, dex 5.
- claude-code (final2-, after the quota reset): matrix 16 pass / 2 gap (rows 13 and 14, the known user-invocability gap), probes 3 pass / 1 fail (probe 93, a harness parser defect), row 04 5/5, lanes 9/9.
Task 21 markers can now be filled from these logs in /tmp/tmp.wiU1YAo8Ky/logs (final-* and final2-*).
Task 21 markers filled from the frozen matrix (1f3b252): INSTALL.md plugin route and matrix table, SURFACE.md row 15, ADR 0019 rows 12 and 18, and findings-final Minor 14. Task 21 stays open on one item only: probe 93 must PASS on claude-code after the events.js fix.
Follow-up fix round done: aa75cef (events.js async subagent), 2aa6d91 (quota GAP needs a clean non-zero exit), b564167 + 2dafef1 (narrowed CLI binds, allowlist gaps), 3db9ba3 (prose). Probe 93 re-run live on claude-code: 4 pass, 0 fail, 0 gap, and sub_output carries the lens finding naming app/users_controller.rb:3.
Exit gate: I ran HOME="$(mktemp -d)" scripts/check-all myself at HEAD: ALL GREEN. Free rows: claude-code 4 pass / 2 gap, opencode 6 pass, codex 4 pass / 2 gap (rows 13 and 14, no runtime check on those harnesses).
Remaining: close task 21's wording, then the completion report.
User challenged the two claude-code GAPs (rows 13 and 14). Checked against the official docs: disable-model-invocation keeps the description out of the model's context AND the runtime blocks the call if the model tries, and claude plugin eval supports a tool_used grader with min 0, max 0, arm both as the documented "must not invoke" check. /skills shows a user-only badge; claude plugin details does not expose the flag (confirmed on the CLI).
