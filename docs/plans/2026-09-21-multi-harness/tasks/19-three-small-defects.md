...
1. **Dead guard tests.** In `lib/git-guard.test.js`, `process.exit` runs
   before the `DEBT #46` block, so its assertions never run. They must run
   and count. Counted in this worktree: **five**, lines 115 to 119, four
   `allowed` and one `blocked`. Design A7 and the task 12 ledger said four;
...
3. **A git command fed to a shell as a heredoc body escapes the guard.**
   ...
   `bash <<EOF` followed by `git branch -D x` is allowed. When ANY consumer in
   the heredoc's pipeline is a shell, the body is commands and must be
   inspected: `cat <<EOF | bash` feeds the body to `bash` just as surely as
   `bash <<EOF` does. The same holds for a here-string, `bash <<< "..."`. When
   no consumer in the pipeline is a shell, such as `cat > file <<EOF` or
   `cat <<EOF | grep x`, the body stays data, exactly as `lib/heredoc.test.js`
   requires today.
...
- Produces: `stripHeredocs(command) -> string`, which keeps a heredoc's body,
  or a here-string's word, when any command in the pipeline it belongs to is a
  shell interpreter: `bash`, `sh`, `zsh`, `dash` or `ksh`. This includes an
  absolute path to one of them, forms with flags such as `bash -s` or `sh -e`,
  and a shell behind any of the wrappers `sudo`, `env`, `timeout <n>`, `nohup`
  or `exec`. Every other heredoc body is still dropped.
...
- The five moved assertions may fail once they run. That is the finding. Fix
...
- [ ] `bash <<EOF` with body `git branch -D x` is refused. So are `sh <<'EOF'`, `/bin/bash -s <<EOF`, `sudo bash <<EOF`, `cat <<EOF | bash`, `timeout 5 bash <<EOF`, `nohup bash <<EOF` and `exec bash <<EOF` with the same kind of body, and `bash <<< "git branch -D x"`
- [ ] `cat <<EOF | grep git` is still allowed: a pipeline with no shell in it keeps the body as data
...
Register it in `scripts/check-all`, on the line directly after
`run install-claude-code ...`. Tasks 14, 16 and 20 each add a line beside a
...
- `heredoc.test.js`: FAIL on "bash reading a heredoc". Report which of the
  new blocked cases already pass today, if any.
