...
- [ ] ADR 0019 records the Codex classifier as a per-binary flag allowlist, as task 14 round 3 built it. It also records the one named limit: a reviewed repository's own `.git/config` `diff.external` or `textconv` runs on a plain `git diff`, and a read-only agent cannot set that config, because it cannot write
