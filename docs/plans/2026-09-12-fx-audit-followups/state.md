...
Task 05: fix round 1/5 (3 addressed, 1 open; commits 0dbb083..9b5f4a1). Re-review:
`.fx/2026-09-12-fx-audit-followups/findings/05-opencode-install-rereview-1.md`. It
confirmed the shared `points_into_fx_skills` check, the corrected `INSTALL.md` text, the
here-string in the test, and `README.md:225` still naming the test. It raised one new
item as Critical: `--dry-run` skips the refusal checks, so a dry run exits 0 where the
real install refuses.

Checked by the controller against both commits:
- The dry-run skip predates the fix. At `9b5f4a1^` the same-named real entry refusal
  already sat after `if args.dry_run: continue` (lines 193-196); the fix put the new
  foreign-link refusal in the same place (`scripts/fx-opencode-install:204-211`).
- Every refusal fires partway through writing. The skill refusals at `:208` and `:211`
  come after stale links are removed at `:197-199` and after earlier skills are linked.
  The command refusal at `:122` comes after the skill, `references` and agent links.

Ruling: the item is Important, not Critical, and task 05 enters fix round 2, resuming its
implementer. Why not Critical: no refusal deletes or overwrites an entry fx did not
create. Why Important: the dry run, which the install guide and step 5 use against a
real configuration, can promise success where the install refuses, and a refused install
leaves the destination half-migrated. Required: every refusal condition (a real
`skills` file, a same-named real entry, a same-named link fx did not create, a command
file not generated by the installer, a rewritten path that does not exist) is checked in
one pass before anything is written, in both modes, and a refusal exits non-zero naming
the entry. Cost if wrong: a third round. Caught by the scoped re-review of round 2.

Task 05: fix round 2 committed as `35666a8`, two files. Verified by the controller in a
scratch destination under the job directory holding a stale fx link `fx-gone` and a
same-named foreign link `fx-tdd`:
- `--dry-run` exits 1 with `refusing to replace a link fx did not create`, naming the
  entry;
- the real install exits 1 with the same message, and afterwards the destination holds
  only `skills/` with `fx-gone` still a link: no `references`, command or agent entry
  was written;
- the fix report shows RED for the three new cases before the change, and a green
  `scripts/check-all`;
- the commit message is clean.

Re-review package `.fx/2026-09-12-fx-audit-followups/review/task-05-fix-2.diff`; re-review
dispatched on the cheapest tier, verdicting each of the five refusal conditions against
the first write.
