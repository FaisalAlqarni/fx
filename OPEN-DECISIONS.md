...
critique, adjust. Would have meant absorbing impeccable's scripts and hook: now moot unless rebuilt from scratch or via chrome-devtools MCP.
...
If motion comes back into scope, take **`motion-foundations` principles only**: duration scales, easing curves, what to animate and what never to. Those are
