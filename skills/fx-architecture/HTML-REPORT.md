...
that tab. It is a throwaway local report, not an app, so the exposure is small
— but if it ever matters, the fix is to **vendor both libraries** into the
...
A fresh file per run. Then open it, and **print the absolute path regardless** —
a silent failure to open leaves the path as the fallback:
