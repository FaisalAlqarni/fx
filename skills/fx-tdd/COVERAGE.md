...
Sources:
- `superpowers:test-driven-development` @ 6.3.0 — claims 1–101
  (+ its `writing-good-tests.md` sidecar, read in full)
- `mattpocock:tdd` @ 1.2.3 — claims 102–128 (+ `tests.md`, `mocking.md`)
- `soe:test-driven-development` (your fork) — claims 129–195
...
## superpowers:test-driven-development (1–101)

| Claims | Verdict | Note |
|---|---|---|
| 1–2 | S | Frontmatter rewritten; triggers name the file extensions and verbs for this stack set |
| 3–7 | K | Write first / watch it fail / minimal code · the core principle · letter-vs-spirit |
| 8–11 | K | **When to Use — Always** (features, bug fixes, refactoring, behavior changes). Restored |
| 12–14 | K | **Exceptions — ask first** (throwaway prototypes, generated code, config files). Restored |
| 15 | K | "skip TDD just this once" is rationalization |
| 16–22 | K | Iron Law; delete it; all four no-exceptions clauses; implement fresh |
| 23 | S | DOT cycle diagram → prose RED → verify → GREEN → verify → REFACTOR, every edge covered |
| 24–29 | K | Write one minimal test; the good/bad annotations; one behavior, clear name, real code |
| 30–35 | K | Verify RED MANDATORY; all three confirmations; passes → fix the test; errors → fix and re-run |
| 36–39 | K | GREEN minimal; both annotations; don't add features / refactor / improve beyond the test |
| 40–45 | K | Verify GREEN MANDATORY; all three confirmations; fix code not test; fix other failures now |
| 46–50 | K | Refactor after green only, all three actions, stay green, no new behavior, repeat |
| 51–53 | K | **Good Tests table — Minimal / Clear / Shows intent, with both examples.** Restored |
...
| 56–58 | R | Assert on real behavior not mock behavior · test-only code in test utilities · understand a dependency's side effects before mocking |
| 59–69 | K | **All 11 rationalization rows**, full reality text |
| 70–82 | K | **All 13 red flags.** 7 were missing from the draft |
| 83 | K | "All of these mean: delete the code, start over" |
| 84–91 | K | All 8 checklist items, **including "edge cases and errors covered"** which the draft dropped |
| 92 | K | "Can't check every box? You skipped TDD." |
| 93–96 | K | All 4 when-stuck rows |
| 97–98 | K | Debugging integration; never fix a bug without a test |
| 99–101 | K | Final rule, both lines, and "no exceptions without permission" |
...
## mattpocock:tdd (102–128)

| Claims | Verdict | Note |
|---|---|---|
| 102–103 | S | Frontmatter merged |
| 104–106 | K | The loop; this is the reference that makes it produce tests worth keeping; every section applies every cycle |
| 107 | K | Read `CONTEXT.md` so test names match the domain language; respect ADRs |
| 108–110 | K | **Behavior through public interfaces · "code can change entirely, tests shouldn't" · "a good test reads like a specification"** — all three restored |
| 111 | R | `tests.md` + `mocking.md` → folded into `good-tests.md`, examples rewritten in Ruby |
| 112–113 | K | Seam definition; tests live at seams, never against internals |
| 114–117 | K | Test only at pre-agreed seams; write them down and confirm; **no test at an unconfirmed seam**; the can't-test-everything reason |
| 118 | K | The question to ask |
| 119–120 | C | Interface shape in question → `codebase-design.md`; "a reference to consult, not a session to run" |
...
| Claims | Verdict | Note |
|---|---|---|
| 129–130 | S | Frontmatter merged |
| 131 | S | `Gate type: verification` per `soe:gate-classification` — soe's mode system is not absorbed |
| 132–141 | K | Duplicates of superpowers 3–15, already kept |
| 142 | S | DOT diagram → prose, Step 0 included |
| 143–147 | K | **Step 0 — user story + not-implemented stubs, the right-reason rationale, skip for bug fixes.** Stub syntax extended to Ruby/C#/Kotlin/Swift/TS |
| 148–150 | K | Duplicates of RED / verify RED |
...
| 153–156 | K | **Runtime RED vs compile-time RED, the not-caused-by-unrelated-errors constraint, and "a test never compiled and executed does not count as RED".** The single most important restoration in this skill — without it the loop rejects correct RED on C#, Kotlin and Swift |
| 157–165 | K | Duplicates of the RED/GREEN/REFACTOR rules |
| 166–167 | S | **80% / 100% coverage thresholds** → no fixed gate. advantage-backend has no SimpleCov, so it is unenforceable; replaced by "every new public method has a test at a confirmed seam", plus a stack-profile `coverage_command`/`coverage_floor` when one exists |
...
| 170–172 | K | Debugging integration; the test proves the fix and prevents regression; never fix a bug without a test |
| 173–183 | K | Red flags — subset of superpowers' 13, already kept |
| 184–190 | K | Checklist — merged with superpowers'; **"valid RED"** wording taken from here |
| 191–193 | K | Final rule |
...
Claims 153–156. My draft carried superpowers' *"the test **fails**, and does not
