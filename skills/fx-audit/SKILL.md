...
- **It edits no code.** It writes the audit's documents into the slug
  directory, working files under `.fx/`, a reference worktree under
...
- **Queue behaviour beyond unbounded enqueue has no dedicated pass.**
  `fx-lens-pipeline` hunts unbounded enqueue only. The gap report judges
...
**Done when** every heading of the skeleton is written, both added header lines
are present, and every top-level entry of the scope is either mapped or listed
...
1. **Resolve the recorded reference exactly as given.**
...
**Done when** every count the template states holds, and every row carries a
file and a line you opened.
...
here, with the user, never inside the subagent. Stop. When the user replies,
append a `## Phase 3 gate choice` section to the end of `03-gaps.md` listing
each candidate taken up by its title, or `none`, before Phase 4 starts.
...
1. **Decide whether the architecture is sound.** It is sound when the verdict
   ...
   `fx-lens-pipeline` reported nothing Critical or Important, and `03-gaps.md`'s
   Phase 3 gate choice reads `none`. Sound is an outcome, not a failure to find
   work: append a `## Phase 4 verdict: sound` section to the end of
...
2. Otherwise write **`design.md`**, under exactly that name: the design
   ...
   is argued from `03-gaps.md`'s rows. Each candidate the gate choice took up is
   argued under Implementation Decisions, and shows in the proposed folder
   tree and the per-module verdict.
