...
- **Spike**: a feasibility question ("can we…", "is it possible…", "quick and
  ...
  question and the probe in 2 to 3 sentences, get a nod, then find out as cheaply
...
2. Present the question + probe plan: 2 to 3 sentences
...
**Ask 2 to 4 related questions per round: one topic per round.** Not one at a
...
driving them. When a question is **behavioural** rather than pictorial ("does this transition hold up when the two events race", "which of these flows
...
Propose **2 to 3 approaches with trade-offs.** Present them conversationally.
...
to 200 to 300 words if nuanced. **Ask after each section whether it looks right so
far.**
...
5. **Prose check**: does it read like a brochure? Inflated claims, "it's not X
   it's Y", stock vocabulary, a sales register. If so, invoke the fx-humanize
   lane on it. A design document is the artifact the user reads most closely, and it
