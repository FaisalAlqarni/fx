...
| 21–22 | S | `docs/superpowers/plans/YYYY-MM-DD-<feature>.md` → `docs/plans/YYYY-MM-DD-<slug>/plan.md`, always a directory |
| 23–25 | K | Scope check; one plan per subsystem; each produces working software alone |
| 26–39 | K | File-structure section entire, incl. both reasons, "files that change together live together", "split by responsibility not layer", existing-codebase rules |
| 40–43 | K | Right-sizing: smallest unit with its own test cycle worth a reviewer's gate |
| 44–49 | K | Bite-sized steps, one action each |
...
| 88–94 | K | All six No-Placeholders failures |
...
| Claims | Verdict | Note |
|---|---|---|
| 128–129 | S | Frontmatter merged into `fx-plan`'s |
...
## soe:adversarial-review (195–267)

| Claims | Verdict | Note |
|---|---|---|
| 195–202 | A | Frontmatter → the agent's own, incl. both modes and the trigger phrases |
| 203–206 | A | Single source of truth; deliberately adversarial; not here to agree; find it while it is cheap to fix |
| 207 | A | Announce line |
| 208–211 | S | Invocation paths (`/soe:critique`, the soe agent, the orchestrator's `EVALUATE_PLAN`) → `fx-plan` step 7 and `/fx:critique` |
| 212–221 | A | **All ten posture rules** — assume flawed, "looks good" is a failure, attack the artifact not the author, don't soften, don't pad, don't invent filler, findings must be real and specific, prefer fewer sharper, **if it wouldn't change the artifact it isn't a finding** |
| 222–231 | A | All seven lenses with their definitions, correct-pattern-usage, cargo-cult rule, **earlier lenses dominate** |
| 232–237 | A | **Design mode entire** — target, gaps (incl. error/concurrency/partial-failure/resume), inconsistencies, missing pieces, pattern misuse, quality-lens catch-all |
| 238–245 | A | Plan mode: target, does-everything-design-mode-does, **faithfulness / drift / gaps / scope creep**, missing-design fallback, flag the missing cross-reference as a limitation |
| 246–251 | A | Numbered findings, one line each, severity order, the output template, plan-internal vs cross-reference callout |
| 252–257 | A | The three options and their exact wording; "Which?" |
| 258 | S | "judgment gate per `soe:gate-classification`" → stated as "this is a judgment gate" without the soe mode system |
| 259–262 | K | **The autonomous branch** — restored into `fx-plan` step 7: no human to pick → one bounded revision, log findings and dispositions to `state.md`, methodology identical, only "who decides" changes |
| 261 | S | `.soe/tracks/{id}/decision-log.md` → `docs/plans/<slug>/state.md` |
| 263–267 | A | **All five red flags** |
...
- The granularity quiz (160–167) — cut, one checkpoint instead of two
