...
description: >
  ...
  this", "look it up", "what do the docs say", "read the spec", "check the RFC",
  "how does <library> actually do this", "find out what's changed in v3". For
  facts that live in this repo, dispatch `Explore` instead; that is not this.
  Skip it and the answer comes from memory about a library that has since
  changed.
---
