...
Sources:
- `mattpocock:code-review` @ 1.2.3 — claims 1–66
- `superpowers:requesting-code-review` @ 6.3.0 — claims 67–98
  (+ its `code-reviewer.md` template, read in full)
- `superpowers:receiving-code-review` @ 6.3.0 — claims 99–183
...
## mattpocock:code-review (1–66)

| Claims | Verdict | Note |
|---|---|---|
| 1–2 | S | Frontmatter rewritten; adds the fx-architecture boundary ("for EXISTING code with no diff, use fx-architecture") |
| 3–5 | K | Two-axis definition — Standards and Spec |
| 6–8 | K | Parallel subagents; the don't-pollute-each-other's-context reason; the skill aggregates |
| 9–10 | S | "tracker should have been provided / run `/setup-matt-pocock-skills`" → `/fx:setup`; the tracker is `docs/plans/` |
| 11–13 | K | Pin the fixed point; ask if not specified |
| 14–15 | K | `git diff <fp>...HEAD` **three-dot**, and the merge-base reason |
...
| 33–44 | F | **All 12 smells**, verbatim, with their fixes |
| 45 | K | Spawn both subagents in parallel |
| 46–48 | K | Standards brief inputs; **the baseline pasted in full**; the "no other access" reason |
| 49–50 | K | The Standards brief verbatim, incl. the 400-word cap |
| 51–54 | K | The Spec brief inputs and text verbatim, incl. the 400-word cap |
| 55 | K | Spec missing → skip that subagent and note it in the report |
| 56–57 | K | Aggregate under separate headings, verbatim or lightly cleaned |
| 58–59 | K | **Do not merge or rerank**, and the reason |
| 60 | K | One-line summary: totals per axis, worst issue within each axis |
| 61–62 | K | **No single winner across axes**, and the reason |
| 63–66 | K | Why two axes; both worked examples; "reporting them separately stops one axis from masking the other" |

## superpowers:requesting-code-review (67–98)
...
| 91–94 | K | **All four "Never" rules.** Three were missing from the draft |
| 95–97 | K | If the reviewer is wrong: technical reasoning · show the code/tests that prove it works · request clarification |
...
## superpowers:receiving-code-review (99–183)

| Claims | Verdict | Note |
|---|---|---|
| 99–100 | S | Frontmatter; becomes a reference, not a selectable skill |
| 101–104 | K/V | Technical evaluation not emotional performance; verify before implementing; ask before assuming; correctness over comfort. **Short form also in the preamble, so subagents inherit it** |
| 105–110 | K/V | The six-step response pattern |
| 111–113 | K/V | The three forbidden responses |
| 114–117 | V | The four "instead" instructions |
| 118–123 | K/V | Unclear items → STOP; ask; both reasons; the wrong/right example |
| 124–127 | V | From the user: trusted, still ask if scope unclear, no performative agreement, skip to action |
| 128–132 | V | The five external-reviewer checks |
| 133–135 | V | Seems wrong → push back · can't verify → say so with the exact phrasing · conflicts with a prior decision → stop and discuss |
| 136 | V | "External feedback — be skeptical, but check carefully" |
| 137–140 | K/V | The YAGNI grep, both branches, and "if we don't need this feature, don't add it" |
| 141–146 | K/V | Clarify first; the three-tier order; test each; verify no regressions |
| 147–152 | K/V | All six push-back triggers |
| 153–156 | V | How to push back — reasoning not defensiveness, specific questions, reference tests/code, escalate if architectural |
| 157–158 | V | **"If you're uncomfortable pushing back out loud: name that tension, then tell them anyway"** |
| 159–161 | V | The three allowed acknowledgments |
| 162–169 | K/V | The five forbidden phrases incl. **"ANY gratitude expression"**; the actions-speak reason; **"if you catch yourself about to write 'Thanks': DELETE IT"** |
| 170–175 | V | Correcting your own push-back: two allowed forms, three forbidden behaviors, state it factually and move on |
| 176–182 | V | All seven common-mistake rows |
