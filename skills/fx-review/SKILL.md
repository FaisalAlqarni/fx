...
`fx-lens-database`'s job; unbounded enqueue against consumer backlog (a
producer or scheduled run adding work with no depth check, high-water mark or
admission control) is `fx-lens-pipeline`'s job; bundle size and rendering
